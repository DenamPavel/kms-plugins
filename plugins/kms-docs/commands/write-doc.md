...
Read the `documentation-pipeline` skill now and follow it as the orchestrator. A standalone `/write-doc` run infers the execution intent from page existence (existing page → `audit`, absent page → `write`) and takes no explicit intent argument. In short:
...
2. Run the stages in order. The gate and stage shape is mode-dependent: `user-guide` and `maintainer` modes run ground → GATE 1 → draft → parallel cross-model review → revise → GATE 2; `agents-md` mode runs ground → GATE 1 → distill (via `doc-agents-md`) → single result gate. For the standard two-gate flow (`user-guide` and `maintainer`): ground (the mode's investigator) → GATE 1 → draft (`doc-writer`) → parallel cross-model review (`doc-fact-checker`, `doc-editor`, the mode's coverage critic) → revise (`doc-reviser`) → GATE 2.
3. Honor all human gates. Do not overwrite a live page without final approval (GATE 2 for standard modes, result gate for agents-md).
